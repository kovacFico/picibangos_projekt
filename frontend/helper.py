...
def create_new_event_form(username):
    ...
    with st.form(key='Create Event'):
        ...
        if create_event_button:
            ...
            elif date_start_end[0] == date_start_end[1] and event_start_time == event_end_time:
                st.error("Event can't have a druation of 0.")
            else:
                post_string = "{\"event_name\": \"" + event_name + " "  + event_color +"\", \"starts_at\": \"" + date_start_end[0].strftime('%Y-%m-%d') + "T" + event_start_time.strftime("%H:%M:%S")+"\", \"ends_at\": \"" +date_start_end[1].strftime('%Y-%m-%d') + "T"+event_end_time.strftime("%H:%M:%S")+"\", \"attendees\": " +format_list_to_string(friends_attending) +", \"teams\": "+format_list_to_string(teams_attending) +"}"
                post_string=post_string.replace("[\"\"]",'[]')
                requests.post(url = "http://127.0.0.1:8000/create_event?user_name="+username, data=post_string)
...
def create_new_team_form(username):
    ...
    if not user_friends:
        ...
        with st.form(key='Create Team'):
            ...
            team_description=st.text_input("Description",max_chars=100,key="team_description")
            ...
            if team_button:
                if not team_name:
                    ...
                    requests.post(url = "http://127.0.0.1:8000/create_team?user_name=" + username,data = 
                        "{\"team_name\": \"" + team_name + 
                        "\", \"description\": \"" + team_description +
                        "\", \"members\": " + format_list_to_string(team_members) +
                        "}")
...
def get_user_events(username):
    ...
    for event in user_events['events']:
        ...
        all_events += "{ \"title\": \"" + event['event_name'][:-7] + "\", \"color\": \""+ event['event_name'][-7:] +"\", \"start\": \""+ event['starts_at'] +"\", \"end\": \""+ event['ends_at'] +"\", \"event_id\": \""+ str(event['event_id']) + "\"},"
    for team in user_events['teams']:
        team_events = requests.get(url = "http://127.0.0.1:8000/team/" + team['team_name']).json()
        if team_events['events']:
            for team_event in team_events['events']:
                if team_event['event_id'] not in existing_events:
                    ...
                    all_events += "{ \"title\": \"" + team_event['event_name'][:-7] + "\", \"color\": \""+ team_event['event_name'][-7:] +"\", \"start\": \""+ team_event['starts_at'] +"\", \"end\": \""+ team_event['ends_at'] +"\", \"event_id\": \""+ str(team_event['event_id']) + "\"},"
...
def sidebar_form(username):
    st.markdown("PIČBANGOŠ PROJEKT")
    my_teams = ["Home"] + get_user_teams(username)
    ikone = ['house']
    for x in range(len(my_teams)):
        ikone.append('s')
    selected_team = option_menu(None,my_teams,icons=ikone,styles={"container":{"background-color": "#f0f2f6"}})
...
def sign_up_mail_password_check(username,mail,p1, p2):
    ...
    for x in username:
        if x.isupper():
            upper_letter = True
    if re.fullmatch(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,7}\b', mail) == None:
        ...
        if res != "{\"detail\":\"User with username: "+username+" doesn't exist.\"}":
        ...
        elif any(char.isupper() for char in p1) == False or any(char.islower() for char in p1) == False:
            st.warning("Password must contain at least one upper case letter and one lower case letter")
            ...
            requests.post(url = "http://127.0.0.1:8000/register", data="{\"user_name\": \"" + 
                          username + "\",\"email\": \"" + mail + 
    ...
    with add_friends_column:
        with st.form(key="Add_new_friends"):
            ...
            if add_friend_button:
                friend_response = requests.get(url = "http://127.0.0.1:8000/account/"+add_friend).text
                if friend_response.find("user_name") == -1:
                    st.error("User doesn't exist")
                else:
                    friend_user_name = friend_response[friend_response.find(":")+2:friend_response.find(",")-1]
                    if friend_user_name in user_friends:
                        st.warning("User " + friend_user_name + " is already your friend.")
                    elif friend_user_name == username:
                        st.warning("You can't add yourself as a friend.")
                    else:
                        user_friends.append(friend_user_name)
                        requests.post(url = "http://127.0.0.1:8000/user/" + username + "/friends", data = format_list_to_string(user_friends))
                        user2_friends = get_user_friends(friend_user_name)
                        if not user2_friends:
                            user2_friends = []
                        user2_friends.append(username)
                        requests.post(url = "http://127.0.0.1:8000/user/" + friend_user_name + "/friends", data = format_list_to_string(user2_friends))
...
def calendar_form(username,selected_events):
    ...
    with calendar_column:
        mode = st.selectbox("Calendar Mode:",("daygrid","timegrid","timeline","list","multimonth"))
...
def event_pop_up(state):
    res = requests.get(url = "http://127.0.0.1:8000/event/" + state["eventClick"]["event"]['extendedProps']['event_id']).json()
    teams_attending = ""
    users_attending = ""
    if res['teams']:
    ...
    if start_date == end_date:
        ...
        with event_columns[1]:
            st.markdown("From " + state["eventClick"]["event"]["start"][0:10].replace("T", " ") + " to " + state["eventClick"]["event"]["end"][0:10].replace("T", " "))
    ...
    {
        border:7px double """ +state["eventClick"]["event"]["backgroundColor"] + """;
        border-radius:5px;
        };
